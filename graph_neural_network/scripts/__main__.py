...
def objective(trial):
    ...
    number_conv_layers = trial.suggest_int("number_conv_layers", 2, 4)
    h_channel = trial.suggest_categorical("h_channel", [2, 4, 8])
    b_size = trial.suggest_categorical("b_size", [32, 64, 128])
    lr = trial.suggest_categorical("lr", [0.01, 0.001, 0.0001])
    dropout_probability = trial.suggest_float("dropout_probability", 0.1, 0.5, step=0.1)
    ...
    val_loader = DataLoader(training_dataset[data_partition:13656], batch_size=b_size, shuffle=True, drop_last=True)
    ...
    model = TestModel(dataset=training_dataset, device=device, batch_size=b_size,
                      ...
                      hidden_channels=h_channel, attention_heads= attention_heads).to(device)
    ...
    wandb.init(project="CAFR_Exploring_FeastNetConv_on_Intersecting_Machining_Features_Experiment_12288_Models_",
