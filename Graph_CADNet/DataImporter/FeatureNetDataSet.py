...
class FeatureNetDataSet(InMemoryDataset):
    ...
    def process(self):
        ...
        for step, file in enumerate(dir_list):
            file_labels = []
            if file[0] =='_':
                file_labels.append(int(file.split("_")[1]))
                file_labels.append(int(file.split("_")[2]))
            else:
                graph_label = file[:2] if (file[2] == '_') else file[0]
                file_labels.append(int(graph_label))

            self.data_list.append(create_graph(path + '/' + file, file_labels))
